...
function InventoryManagement() {
  ...
  const toggleAllRows = () => {
    if (selectedRows.length === inventoryData.length) {
      ...
      setSelectedRows(inventoryData.map(item => item.id))
    }
  }
        ...
        <div className="hidden md:block bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
            ...
            <table className="w-full">
              <thead className="bg-gray-50 dark:bg-gray-700 border-b border-gray-200 dark:border-gray-600">
                <tr>
                  ...
                  <th className="px-4 py-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Kho</th>
                  <th className="px-4 py-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Vị trí</th>
                  <th className="px-4 py-3 text-center text-xs font-semibold text-gray-600 uppercase tracking-wider">Tồn thực tế</th>
                  <th className="px-4 py-3 text-center text-xs font-semibold text-gray-600 uppercase tracking-wider">Tồn khả dụng</th>
                  <th className="px-4 py-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Hạn sử dụng</th>
              ...
              <tbody className="divide-y divide-gray-200">
                {inventoryData.map((item) => (
                  <tr 
                    key={item.id} 
                    className={`${getRowColor(item.status)} hover:opacity-80 transition-opacity border-b border-gray-200 dark:border-gray-700`}
                  >
                    ...
                    <td className="px-4 py-3">
                      <span className="text-sm font-mono font-medium text-gray-900 dark:text-white">{item.code}</span>
                    </td>
                    <td className="px-4 py-3">
                      <Link to={`/products/${item.code}`} className="text-sm font-medium text-primary hover:underline text-left">
                        {item.name}
                      </Link>
                    </td>
                    <td className="px-4 py-3">
                    ...
                    <td className="px-4 py-3 text-center">
                      <span className="text-sm text-gray-700">{item.availableStock}</span>
                    </td>
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-1">
                        <Calendar className="w-4 h-4 text-gray-400" />
                        <span className={`text-sm ${isExpiringSoon(item.expiryDate) ? 'text-red-600 font-medium' : 'text-gray-700'}`}>
                          {item.expiryDate}
                        </span>
          ...
          <div className="px-4 py-3 border-t border-gray-200 dark:border-gray-700 flex items-center justify-between">
            <div className="text-sm text-gray-700 dark:text-gray-300">
              Hiển thị <span className="font-medium">1</span> đến <span className="font-medium">{inventoryData.length}</span> của <span className="font-medium">{inventoryData.length}</span> kết quả
            </div>
            <div className="flex gap-2">
        ...
        <div className="md:hidden space-y-4">
          {inventoryData.map((item) => (
            ...
            >
              ...
              <h3 className="text-sm font-medium text-primary mb-3">{item.name}</h3>
              ...
              <div className="space-y-2 text-sm">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-bold text-gray-900 dark:text-white">{item.actualStock}</span>
                </div>
                <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  <Calendar className="w-4 h-4 text-gray-400" />
                  <span className={isExpiringSoon(item.expiryDate) ? 'text-red-600 font-medium' : 'text-gray-700'}>
                    {item.expiryDate}
                  </span>
                  {isExpiringSoon(item.expiryDate) && (
                    <AlertTriangle className="w-4 h-4 text-red-500" />
